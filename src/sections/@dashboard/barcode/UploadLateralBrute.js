...
function UploadImg({ list1, list2 }) {
    ...
    useEffect(() => {
        const getLateralBrute = async () => {
            await NewsServices.lateralBrute()
                .then((res) => {
                    console.log(res)
                    setJson(res.data)
                })
...

            {
                load && json && <Grid item container rowSpacing={3} columnSpacing={3} sx={{ marginTop: '3%' }} > 
                    <Grid item md={12}>
                        ...
                        <Alert sx={{ cursor: 'pointer' }} onClick={() => open3(json.blacklist_ip)} severity="error"> Blacklisted IP address <strong>{json.blacklist_ip}</strong></Alert>
                        {editSinglePerson && <>
