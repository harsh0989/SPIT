...
function UploadImg({ list1, list2 }) {
    ...
    useEffect(() => {
        const getBotnets = async () => {
            await NewsServices.botnet()
                .then((res) => {
                    console.log(res)
                    setJson(res.data)
                })
...

            {
                load && json && <Grid item container rowSpacing={3} columnSpacing={3} sx={{ marginTop: '3%' }} >
                    <Grid item md={12}>
                        ...
                        <Alert sx={{ cursor: 'pointer' }} onClick={() => open3(json.all_ips[0])} severity="error"> Blacklisted ports {
                            json.blacklist_port.map((por) => {
